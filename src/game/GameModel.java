...
public class GameModel {
    ...
    public GameModel(Logger logger) {
        ...
        // No automatic ship creation - tests expect an empty model
    }
    ...
    public void spawnObjects() {
        Ship ship = getShip();
        boolean spawnAsteroid = random.nextInt(100) < spawnRate;// Check if an asteroid should spawn
        int asteroidX = random.nextInt(GAME_WIDTH);// Determine asteroid x-coordinate

        // Only add the asteroid if it should spawn and doesn't collide with ship
        if (spawnAsteroid && (ship == null || ship.getX() != asteroidX || ship.getY() != 0)) {
            addObject(new Asteroid(asteroidX, 0));
        }

        boolean spawnEnemy = random.nextInt(100) < spawnRate * ENEMY_SPAWN_RATE;// Check if an enemy should spawn
        int enemyX = random.nextInt(GAME_WIDTH);// Determine enemy x-coordinate
        // Only add the enemy if it should spawn and doesn't collide with ship
        if (spawnEnemy && (ship == null || ship.getX() != enemyX || ship.getY() != 0)) {
            addObject(new Enemy(enemyX, 0));
        }

        boolean spawnPowerUp = random.nextInt(100) < spawnRate * POWER_UP_SPAWN_RATE;// Check if a power-up should spawn
        int powerUpX = random.nextInt(GAME_WIDTH);// Determine power-up x-coordinate
        boolean isShield = random.nextBoolean();// Determine power-up type
        // Only add the power-up if it should spawn and doesn't collide with ship
